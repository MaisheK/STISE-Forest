...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Insert data into `victim55` table
    $insert_query = "INSERT INTO victim55 (cut_tree, victim, damage_category, treenum) 
                     VALUES ('$cut_tree', '$victim', $damage_category, '$treenum')";
    $insert_result = mysqli_query($dbc, $insert_query);

    if (!$insert_result) {
        die('Error inserting into victim55 table: ' . mysqli_error($dbc));
    }
}

// Output success message for data transfer
echo "The data has been successfully transferred to the victim55 table.<br>";

// Step 2: Update `forest55` table with random volumes based on `treenum` from `victim55` data
$query = "SELECT treenum, damage_category FROM victim55";
$result = mysqli_query($dbc, $query);

if (!$result) {
    die('Error fetching victim55 data: ' . mysqli_error($dbc));
}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Update the `forest55` table using `treenum`
    $update_query = "UPDATE forest55 
                     SET 
