...
// Step 1: Transfer data from `damagetree50` to `victim50`
$query = "SELECT cut_tree, victim, damage_category, treenum FROM damagetree50";
$result = mysqli_query($dbc, $query);

if (!$result) {
    die('Error fetching data from damagetree50: ' . mysqli_error($dbc));
}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Insert data into `victim50` table
    $insert_query = "INSERT INTO victim50 (cut_tree, victim, damage_category, treenum) 
                     VALUES ('$cut_tree', '$victim', $damage_category, '$treenum')";
    $insert_result = mysqli_query($dbc, $insert_query);

    if (!$insert_result) {
        die('Error inserting into victim50 table: ' . mysqli_error($dbc));
    }
}

// Output success message for data transfer
echo "The data has been successfully transferred to the victim50 table.<br>";

// Step 2: Update `Forest50` table with random volumes based on `treenum` from `victim50` data
$query = "SELECT treenum, damage_category FROM victim50";
$result = mysqli_query($dbc, $query);

if (!$result) {
    die('Error fetching victim50 data: ' . mysqli_error($dbc));
}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Update the `Forest50` table using `treenum`
    $update_query = "UPDATE Forest50 
                     SET 
