...
// For example, you can retrieve data from the 'forest60' table and perform calculations:
// Construct the SQL query to retrieve data for all "Cut" trees
$sql = "SELECT treenum, cut_angle, x_coordinate AS x, y_coordinate AS y, height FROM forest60 WHERE tree_status = 'Cut'";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($cutAngle >= 0 && $cutAngle < 90) {
        ...
        $count_query = "SELECT treenum, x_coordinate AS x, y_coordinate AS y FROM forest60 WHERE tree_status != 'Cut' AND x_coordinate > $x0 AND x_coordinate < $x_upper AND y_coordinate > $y0 AND y_coordinate < $y_upper";
    }
    elseif ($cutAngle >= 90 && $cutAngle < 180) {
        // Quadrant II: 90 - 180 degrees
        $x_upper = $x0 + $stemHeight + $buffer;
        $y_upper = $y0 - $stemHeight - $buffer;
        $count_query2 = "SELECT treenum, x_coordinate AS x, y_coordinate AS y FROM forest60 WHERE tree_status != 'Cut' AND x_coordinate > $x0 AND x_coordinate < $x_upper AND y_coordinate > $y0 AND y_coordinate < $y_upper";
    }
    elseif ($cutAngle >= 180 && $cutAngle < 270) {
        // Quadrant III: 180 - 270 degrees
        $x_upper = $x0 - $stemHeight - $buffer;
        $y_upper = $y0 - $stemHeight - $buffer;
        $count_query3 = "SELECT treenum, x_coordinate AS x, y_coordinate AS y FROM forest60 WHERE tree_status != 'Cut' AND x_coordinate > $x0 AND x_coordinate < $x_upper AND y_coordinate > $y0 AND y_coordinate < $y_upper";

    } elseif ($cutAngle >= 270 && $cutAngle < 360) {
        // Quadrant IV: 270 - 360 degrees
        $x_upper = $x0 - $stemHeight - $buffer;
        $y_upper = $y0 + $stemHeight + $buffer;
        $count_query4 = "SELECT treenum, x_coordinate AS x, y_coordinate AS y FROM forest60 WHERE tree_status != 'Cut' AND x_coordinate > $x0 AND x_coordinate < $x_upper AND y_coordinate > $y0 AND y_coordinate < $y_upper";
    }
    ...
    if (!empty($count_query)) {
        ...
        while ($row1 = mysqli_fetch_assoc($result1)) {
            ...
            if (($unknownTree_Y > $y1 && $unknownTree_Y < $y2) ) {
                ...
                $insert_query = "INSERT INTO damagetree60 (cut_tree, victim, damage_category, treenum) VALUES ('$cut_tree_coordinate', '$victim_coordinate', $categoryDamage, '$victim_treenum') ";
