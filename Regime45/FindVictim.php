...
// Step 1: Transfer data from `damagetree` to `victim`
$query = "SELECT cut_tree, victim, damage_category, treenum FROM damagetree";
$result = mysqli_query($dbc, $query);

if (!$result) {
    die('Error fetching data from damagetree: ' . mysqli_error($dbc));
}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $insert_query = "INSERT INTO victim (cut_tree, victim, damage_category, treenum) 
...
echo "The data has been successfully transferred to the victim table.<br>";

// Step 2: Update `tree_data` table with random volumes based on `treenum` from `victim` data
$query = "SELECT treenum, damage_category FROM victim";
$result = mysqli_query($dbc, $query);
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Update the `tree_data` table using `treenum`
